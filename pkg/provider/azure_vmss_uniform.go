...
// uniformScaleSet implements VMSet interface for Azure scale set.
type uniformScaleSet struct {
	*Cloud
...
// newUniformScaleSet creates a new uniformScaleSet.
func newUniformScaleSet(az *Cloud) (*uniformScaleSet, error) {
...
func (ss *uniformScaleSet) getVMSS(vmssName string, crt azcache.AzureCacheReadType) (*compute.VirtualMachineScaleSet, error) {
...
func (ss *uniformScaleSet) getVmssVMByNodeIdentity(node *nodeIdentity, crt azcache.AzureCacheReadType) (*virtualmachine.VirtualMachine, error) {
	...
	getter := func(nodeName string, crt azcache.AzureCacheReadType) (*virtualmachine.VirtualMachine, bool, error) {
		...
		if vm, ok := virtualMachines.Load(nodeName); ok {
			...
			found = true
			return virtualmachine.FromVirtualMachineScaleSetVM(result.resourceGroup, result.virtualMachine,
				virtualmachine.ByVMSS(result.vmssName)), found, nil
		}
...
func (ss *uniformScaleSet) getVmssVM(nodeName string, crt azcache.AzureCacheReadType) (*virtualmachine.VirtualMachine, error) {
...
func (ss *uniformScaleSet) getVmssVMByInstanceID(resourceGroup, scaleSetName, instanceID string, crt azcache.AzureCacheReadType) (*compute.VirtualMachineScaleSetVM, error) {
...
func (ss *uniformScaleSet) GetInstanceIDByNodeName(name string) (string, error) {
...
func (ss *uniformScaleSet) GetZoneByNodeName(name string) (cloudprovider.Zone, error) {
	managedByAS, err := ss.isNodeManagedByAvailabilitySet(name, azcache.CacheReadTypeUnsafe)
	if err != nil {
		klog.Errorf("Failed to check isNodeManagedByAvailabilitySet: %v", err)
...
func (ss *uniformScaleSet) GetPrimaryVMSetName() string {
	return ss.Config.PrimaryScaleSetName
}

// GetIPByNodeName gets machine private IP and public IP by node name.
func (ss *uniformScaleSet) GetIPByNodeName(nodeName string) (string, string, error) {
...
func (ss *uniformScaleSet) getVMSSPublicIPAddress(resourceGroupName string, virtualMachineScaleSetName string, virtualMachineIndex string, networkInterfaceName string, IPConfigurationName string, publicIPAddressName string) (network.PublicIPAddress, bool, error) {
...
func (ss *uniformScaleSet) GetPrivateIPsByNodeName(nodeName string) ([]string, error) {
	ips := make([]string, 0)
	nic, err := ss.GetPrimaryInterface(nodeName)
	if err != nil {
		klog.Errorf("error: ss.GetIPByNodeName(%s), GetPrimaryInterface(%q), err=%v", nodeName, nodeName, err)
...
func (ss *uniformScaleSet) getPrimaryInterfaceID(vm *virtualmachine.VirtualMachine) (string, error) {
...
func (ss *uniformScaleSet) listScaleSets(resourceGroup string) ([]string, error) {
	ctx, cancel := getContextWithCancel()
	defer cancel()
...
func (ss *uniformScaleSet) getNodeIdentityByNodeName(nodeName string, crt azcache.AzureCacheReadType) (*nodeIdentity, error) {
...
func (ss *uniformScaleSet) listScaleSetVMs(scaleSetName, resourceGroup string) ([]compute.VirtualMachineScaleSetVM, error) {
	ctx, cancel := getContextWithCancel()
	defer cancel()
...
func (ss *uniformScaleSet) getAgentPoolScaleSets(nodes []*v1.Node) (*[]string, error) {
...
func (ss *uniformScaleSet) GetVMSetNames(service *v1.Service, nodes []*v1.Node) (*[]string, error) {
...
func (ss *uniformScaleSet) GetPrimaryInterface(nodeName string) (network.Interface, error) {
...
func (ss *uniformScaleSet) getPrimaryNetworkInterfaceConfiguration(networkConfigurations []compute.VirtualMachineScaleSetNetworkConfiguration, nodeName string) (*compute.VirtualMachineScaleSetNetworkConfiguration, error) {
...
func (ss *uniformScaleSet) getPrimaryNetworkInterfaceConfigurationForScaleSet(networkConfigurations []compute.VirtualMachineScaleSetNetworkConfiguration, vmssName string) (*compute.VirtualMachineScaleSetNetworkConfiguration, error) {
	if len(networkConfigurations) == 1 {
		return &networkConfigurations[0], nil
	}
...
func (ss *uniformScaleSet) getConfigForScaleSetByIPFamily(config *compute.VirtualMachineScaleSetNetworkConfiguration, nodeName string, IPv6 bool) (*compute.VirtualMachineScaleSetIPConfiguration, error) {
	ipConfigurations := *config.IPConfigurations
...
func (ss *uniformScaleSet) EnsureHostInPool(service *v1.Service, nodeName types.NodeName, backendPoolID string,
	vmSetNameOfLB string) (string, *virtualmachine.VirtualMachine, error) {
	...
	if vmSetNameOfLB != "" && needCheck && !strings.EqualFold(vmSetNameOfLB, vm.VMSSName) {
		klog.V(3).Infof("EnsureHostInPool skips node %s because it is not in the uniformScaleSet %s", vmName, vmSetNameOfLB)
		return "", nil, nil
	}
	...
	newVM := virtualmachine.FromVirtualMachineScaleSetVM(&compute.VirtualMachineScaleSetVM{
	...
	}, virtualmachine.ByVMSS(vm.VMSSName))
	...
	return nodeResourceGroup, newVM, nil
}
...
func (ss *uniformScaleSet) ensureVMSSInPool(service *v1.Service, nodes []*v1.Node, backendPoolID string, vmSetNameOfLB string) error {
...
func (ss *uniformScaleSet) EnsureHostsInPool(service *v1.Service, nodes []*v1.Node, backendPoolID string, vmSetNameOfLB string) error {
	...
	for _, node := range nodes {
		...
		if managedByAS {
			...
			if ss.useStandardLoadBalancer() {
				hostUpdates = append(hostUpdates, func() error {
					_, _, err := ss.availabilitySet.EnsureHostInPool(service, types.NodeName(localNodeName), backendPoolID,
						vmSetNameOfLB)
					return err
				})
				continue
		...
		nodeResourceGroup, vm, err := ss.EnsureHostInPool(service, types.NodeName(localNodeName), backendPoolID,
			vmSetNameOfLB)
		if err != nil {
		...
		nodeVMSSMetaInfo := vmssMetaInfo{vmssName: vm.VMSSName, resourceGroup: nodeResourceGroup}
		if v, ok := nodeUpdates[nodeVMSSMetaInfo]; ok {
			v[vm.InstanceID] = *vm.AsVirtualMachineScaleSetVM()
		} else {
			nodeUpdates[nodeVMSSMetaInfo] = map[string]compute.VirtualMachineScaleSetVM{
				vm.InstanceID: *vm.AsVirtualMachineScaleSetVM(),
			}
		}
...
func (ss *uniformScaleSet) ensureBackendPoolDeletedFromNode(nodeName, backendPoolID string) (string, *virtualmachine.VirtualMachine, error) {
	vm, err := ss.getVmssVM(nodeName, azcache.CacheReadTypeDefault)
	if err != nil {
		if errors.Is(err, cloudprovider.InstanceNotFound) {
...
func (ss *uniformScaleSet) GetNodeNameByIPConfigurationID(ipConfigurationID string) (string, string, error) {
...
func (ss *uniformScaleSet) ensureBackendPoolDeletedFromVMSS(service *v1.Service, backendPoolID, vmSetName string, ipConfigurationIDs []string) error {
	vmssNamesMap := make(map[string]bool)
...
func (ss *uniformScaleSet) EnsureBackendPoolDeleted(service *v1.Service, backendPoolID, vmSetName string, backendAddressPools *[]network.BackendAddressPool, deleteFromVMSet bool) error {
	...
	for i := range ipConfigurationIDs {
		...
		nodeResourceGroup, vm, err := ss.ensureBackendPoolDeletedFromNode(nodeName, backendPoolID)
		if err != nil {
			if !errors.Is(err, ErrorNotVmssInstance) { // Do nothing for the VMAS nodes.
...
func (ss *uniformScaleSet) EnsureBackendPoolDeletedFromVMSets(vmssNamesMap map[string]bool, backendPoolID string) error {
